...
def on_timer(timer):
    if(uart_B.any()):
        a=get_dis()
tim = machine.Timer(Timer.TIMER0, Timer.CHANNEL0, mode=Timer.MODE_PERIODIC, period=200, unit=Timer.UNIT_MS, callback=on_timer, arg=on_timer, start=True, priority=1, div=0)
#task = kpu.load("/sd/out_txm.kmodel")#oupp
...
while(True):
    ...
    chec=check(output)
    if code:
        chec_num=check_num(int(code[0].classid()))#int(labels[int(code[0].classid())])
