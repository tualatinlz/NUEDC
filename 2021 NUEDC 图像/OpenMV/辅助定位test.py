...
class find_b(object):
    ...
    def find(self,img):
        if (ctrl.work_mode==0x05):
            ...
            self.GRAYSCALE_THRESHOLD = [(29, 75, -45, -17, -4, 47)]
        ...
        if blobs:
            ...
            if(ctrl.work_mode==0x09):     #判断为右上角
                self.get_disnew(blobs[largest_blob].w(),blobs[largest_blob].y())
                print("mode you shang:")
                print(blobs[largest_blob].y()+blobs[largest_blob].h())
            ...
            ctrl.work_mode=0x05   #记得改

mode2=find_b()
ctrl.work_mode = 0x05
if(ctrl.work_mode>=0x05):
    sensor.set_windowing((146,67,100,130))  #模式二换window (100,60,120,120)
    sensor.skip_frames(time = 1000)
if(ctrl.work_mode>=0x05):
    sensor.set_windowing((90,30,150,150))  #模式二换window (100,60,120,120)
    sensor.skip_frames(time = 1000)
while(True):
    clock.tick()
