...
void DrvPwmOutInit(void)
{
	...
	/* Set divider to 80M/8=0.1us 精度为0.1*/
	...
	ROM_PWMGenPeriodSet(PWM1_BASE, PWM_GEN_1, 60000); 
	ROM_PWMGenPeriodSet(PWM1_BASE, PWM_GEN_2, 60000); 
...
void DrvMotorPWMSet(int16_t pwm[4])
{
	...
	for(u8 i=0; i<4; i++)
	{
		if(pwm[i]>MAXTHROTTLE - 1) pwm[i] = MAXTHROTTLE - 1;
...
void DrvServoPWMSet(int16_t pwm[4])
{
	...
	{
		if(pwm[i]>MAXTHROTTLE - 1) pwm[i] = MAXTHROTTLE - 1;
		tempval[i] = 133*pwm[i]+3000;//0-180对应3000-27000
	}
	/*配置比较捕获寄存器的预装载值*/
	ROM_PWMPulseWidthSet(PWM1_BASE,PWM_OUT_4,tempval[0]);
	ROM_PWMPulseWidthSet(PWM1_BASE,PWM_OUT_5,tempval[1]);
	ROM_PWMPulseWidthSet(PWM1_BASE,PWM_OUT_6,tempval[2]);
	ROM_PWMPulseWidthSet(PWM1_BASE,PWM_OUT_7,tempval[3]);
}
/**********************************************************************************************************
*函 数 名: Drv_HeatSet
*功能说明: 加热PWM输出值设置
